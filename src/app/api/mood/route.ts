...
export async function POST(request: Request) {
  try {
    ...
    if (!data.mood || !['happy', 'calm', 'fussy', 'crying'].includes(data.mood)) {
...
export async function PATCH(request: Request) {
  try {
    ...
    const existing = await prisma.moodRecord.findFirst({
      where: { id, babyId: baby.id },
    })
    if (!existing) {
    ...
    if (data.mood !== undefined && !['happy', 'calm', 'fussy', 'crying'].includes(data.mood)) {
